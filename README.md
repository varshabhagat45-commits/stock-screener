...
An Anthropic API key — separate from your claude.ai login. Analysis calls here are
billed pay-as-you-go on that key (not part of a Claude.ai subscription).
A Vercel account (or Netlify/Render — steps below are for Vercel, the simplest path).
---
1. Get an Anthropic API key
Go to https://console.anthropic.com and sign in (or create an account).
Add billing (Settings → Billing) — pay-as-you-go, no separate subscription needed.
Settings → API Keys → Create Key. Copy it — you won't see it again after this screen.
...
Go to https://vercel.com and sign up (GitHub login is simplest).
Create a new GitHub repo, upload these three files/folders to it:
`index.html`, `api/analyze.js`, `package.json`.
In Vercel: Add New → Project → Import your new repo.
Before clicking Deploy, open Environment Variables and add:
Name: `ANTHROPIC_API_KEY`
Value: (paste the key from step 1)
Click Deploy. Vercel gives you a live URL like `your-project.vercel.app` —
